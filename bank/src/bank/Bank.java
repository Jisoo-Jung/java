...
//	잔액(통장)
	...
	public Bank(String name, String account, String phone, String password, int money) { //초기화 생성자
		super();
		this.name = name;
...
//	다른 자식 클래스에서 재정의하여 수정하면 안되기 때문에 final을 붙인다.
	public final static Bank checkAccount(Bank[][] arrBank, int[] arCount,  String account) {
		...
		for (int i = 0; i < arrBank.length; i++) { // 은행 수 만큼 반복 //만약 arrBank[3][100]이라면? // i 는 0,1,2 일텐데...
//			for문 안에서 선언하면, 그 영역에서만 사용이 가능하다.
//			하지만 for문 밖에서도 변수를 사용하고 싶다면, 해당 영역에 선언해야 한다.
			int j = 0;
			for (j = 0; j < arCount[i]; j++) { // 각 은행의 고객 수 만큼 반복 
				//j가 arCount[0]보다 작을 때, j가 arCount[1]보다 작을 때, j가 arCount[2]보다 작을 때  
